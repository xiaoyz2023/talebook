<template>
    <v-card>
        ...
        <v-card-text> 共图书记录：{{this.total}} 条，总金额：</v-card-text>
        <v-card-actions>
        ...
        >
            ...
            <template v-slot:item.price="{ item }">
                ...
                >
                    ...
                    <template v-slot:input>
                        <div class="mt-4 text-h6">修改字段</div>
                        <v-text-field v-model="item.price" label="price" counter></v-text-field>
                    </template>
                </v-edit-dialog> 
            </template>
            ...
            <template v-slot:item.storage="{ item }">
                 <v-edit-dialog
                    large
                    :return-value.sync="item.storage"
                    @save="save(item, 'storage')"
                    save-text="保存"
                    cancel-text="取消"
                >
                    {{ item.storage }}
                    <template v-slot:input>
                        <div class="mt-4 text-h6">修改字段</div>
                        <v-text-field v-model="item.storage" label="storage" counter></v-text-field>
                    </template>
                </v-edit-dialog> 
            </template>
