            ...
            <v-card-text v-show="card.show" style="padding: 0">
              <div style="padding: 0 16px 16px">
                ...
                <template v-for="f in card.fields"  >
                  <v-checkbox :key="f.key" small hide-details v-if="f.type === 'checkbox' " :prepend-icon="f.icon" v-model="settings[f.key]" :label="f.label" ></v-checkbox>
                ...
                <template v-for="b in card.buttons"  >
                  <v-btn @click="run(b.action)" color="primary"><v-icon :key="b.label">{{b.icon}}</v-icon>{{b.label}}</v-btn>
                </template>
                ...
                <template v-for="g in card.groups"  >
                  ...
                  <template v-if="settings[g.key]">
                    <template v-for="f in g.fields"  >
                      <v-textarea :key="f.key" outlined v-if="f.type === 'textarea' " :prepend-icon="f.icon" v-model="settings[f.key]" :label="f.label" ></v-textarea>
                      <v-text-field v-else :prepend-icon="f.icon" v-model="settings[f.key]" :label="f.label" type="text"></v-text-field>
                    </template>
                  </template>
                </template>
